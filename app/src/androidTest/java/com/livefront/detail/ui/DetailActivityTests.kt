...
class DetailActivityTests {
    ...
    @get:Rule
    var activityRule: IntentsTestRule<DetailActivity> = object : IntentsTestRule<DetailActivity>(DetailActivity::class.java) {
        override fun getActivityIntent(): Intent {
            ...
            result.putExtra("result", getSampleMovieResponseFromJson().results?.get(0))
            return result
        }
    }
    ...
    fun verifyClickingOnItemStartsDetailIntent() {
        onView(withId(R.id.detailImageFab)).perform(click())
        Intents.intended(IntentMatchers.hasComponent(MovieImageViewActivity::class.java.name))
    }

    @Test
    fun verifyDetailResponseReturns() {
        /**
         * Verifies the API returns and the views we expect to be displayed are displayed
         */
        ...
        activityRule.activity.runOnUiThread({
            ...
            //if we don't do this, we can't scroll a NestedScrollView
            val nestedScrollView = activityRule.activity.findViewById(R.id.detailScrollView) as NestedScrollView
            val params = nestedScrollView.layoutParams as CoordinatorLayout.LayoutParams
        ...
        onView(withId(R.id.detailLanguageImage)).check(matches(isDisplayed()))

...
class UpcomingMovieCall @Inject constructor(private val movieService: MovieService) : MovieCallInterface {
    ...
    /**
     * Fetches upcoming movies! This method is the start of the chain that will eventually update
     * our [MutableLiveData] object with the [MovieResponse] that we recieved from the API
     *
    ...
    override fun fetchMovies(page: Int, liveData: MutableLiveData<MovieResponse?>) {
        movieService.getUpcomingMovies(page)
                ...
                .subscribe({ response ->
                    ...
                    liveData.value = null
                    Log.e(TAG, "An error occurred while fetching upcoming movies", it)
                })
    }

    /**
     * Parses the [MovieResponse] we received from the API for upcoming movies.
     *
    ...
    override fun parseResponse(response: MovieResponse, liveData: MutableLiveData<MovieResponse?>) {
        ...
        if(response.results != null) {
            ...
            Log.e(TAG, "An error occurred while fetching / parsing upcoming movies")
        }
    }

    /**
     * Returns the implementation of [OnLoadMoreListener] for upcoming movies. If our adapter can load more items,
